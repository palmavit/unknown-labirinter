...
int main(int argc, char **argv) {
  ...
  subscribe(subscribers, {"left_back", "right_back","left_right", "right_right", "left_left", "right_left", "right_center", "left_center"}, update_proximity,
            "proximity");
  subscribe(subscribers, {"center"}, update_reflectance,
            "reflectance");
  ...
  // ros::Duration{random_time} - время между вызовами функции управления (control)
  auto timer = node.createTimer(ros::Duration{0.1}, timer_callback);  // ros::Duration{0.5}
